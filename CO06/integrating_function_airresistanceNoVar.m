function dwdt = integrating_function_airresistanceNoVar(t, w)
    DragForceConstant =0.001571664191765;
    mass =4.2;
    g=9.81;
%AUTHOR: Katie Scales,  Date: 14/12/2022
%SUMMARY: Sets up the ODE solver for a air resistance situation with a non
...
%INPUT Variables: 
    ...
    dwdt(2) = (-1*g)-(-1*DragForceConstant*(1/(2*mass)))*(sqrt((ydot^2)+(xdot^2))*ydot);%This is ydotdot
    dwdt(3) = xdot; %this is xdot
    dwdt(4) = -1*DragForceConstant*(1/(2*mass))*sqrt((ydot^2)+(xdot^2))*xdot; %Gives xdotdot
end